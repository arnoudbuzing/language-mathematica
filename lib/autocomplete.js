...
export default class AutocompleteProvider {
  constructor () {
    this.selector = '.source.wolfram'
    this.disableForSelector = '.source.wolfram .comment, .source.wolfram .string'
  ...
  loadCompletions () {
    fs.readFile(path.resolve(__dirname, '..', 'completions.json'), (error, content) => {
      if (error) {
        atom.notifications.addError('Unable to load autocomplete list', {detail: error})
      } else {
